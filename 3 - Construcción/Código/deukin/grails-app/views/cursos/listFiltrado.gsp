	...
	<body>
		...
		<div id="list-configuracionCursoDia" class="content scaffold-list" role="main">
			...
			<label class="col-lg-2"><g:message code="control-label cursos.planEstudio.label" default="Plan de Estudio" /></label>
				<div class="col-lg-4">
	<g:select id="planEstudio.id" name="planEstudio.id" from="${com.deukin.PlanEstudio.list()}" optionKey="id" required="" value="${planEstudio?.id}" class="many-to-one"  onchange="this.form.submit()"/>
	</div>
				<label class="col-lg-2"><g:message code="control-label cursos.cicloLectivo.label" default="Ciclo Lectivo" /></label>
				<div class="col-lg-4">
	<g:select id="planEstudio.id" name="cicloLectivo.id" from="${com.deukin.CicloLectivo.list()}" optionKey="id" required="" value="${cicloLectivo?.id}" class="many-to-one" onchange="this.form.submit()"/>
	</div>
	</div>
	</g:form>
			...
			<table class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<th>${message(code: 'cursos.anio.label', default: 'Año')}</th>
						
						<th>${message(code: 'cursos.cuatrimestre.label', default: 'Cuatrimestre')}</th>
				...
				<g:each in="${configuracionCursoDiaInstanceList}" status="i" var="configuracionCursoDiaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: configuracionCursoDiaInstance?.curso?.materia?.periodo, field: "anio")}</td>
						
						<td>${fieldValue(bean: configuracionCursoDiaInstance?.curso?.materia?.periodo, field: "cuatrimestre")}</td>
